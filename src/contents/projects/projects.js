...
function ProjectContents(props) {
    ...
    return <div className="projects-contents-info-box">
        ...
        <div className="projects-button-box">
            ...
            {detail &&
                (<Modal youtube={props.youtube} md={props.md} closeModal={() => setDetail(!detail)}>
                </Modal>)}
        </div>
    </div>
}
function LeftSide(props) {
    return <div>
        ...
        <div className="left-project-info">
            <ProjectContents youtube={props.youtube} md={props.md} text={props.text}/>
        </div>
    </div>
}
...
export function Projects() {
    ...
    text.push(["2022.03.02~2022.10.01","스미싱 방지 어플리케이션", "시간이 지날수록 높아져 가는 스미싱 피해에 AI를 접목하면 스미싱 피해를 줄일 수 있지 않을까? 라는 생각에서부터 시작된 졸업 프로젝트입니다. \nAI에 관련한 지식이 없어 많은 어려움이 있었고, 사이킷 런 라이브러리를 이용해 AI를 직접 사용해 보았다는 보람을 느낄 수 있었습니다.", "https://github.com/SeniorProjectHoseo2022","https://www.youtube.com/embed/RctHAG5oD1M"])
    text.push(["2022.01.15~2022.01.21","Petmilly", "애완동물 용품에 전문화된 관련한 중고 거래사이트가 없어, 팀장님이 고양이를 기르는데 기호에 맞는 장난감들로 인해 낭비를 하게 되었습니다. \n이러한 문제를 해결할 수 있는 커뮤니티를 형성하고, 초보 반려인들을 위한 퀘스트들을 제공해 더욱 즐거운 반려생활 위한 어플을 제작하였습니다.", "https://github.com/jmg9776/2022.02-Petmily_BackEnd-node.js","https://www.youtube.com/embed/XLCAXPoN0y0"])
    ...
    for (let i in project_img) {
        projectRender.push( <div className="projects-contents-box">
            {<LeftSide key={project_img} num={count+"project"} md={markdown[(count+1)+".md"]} text={text[count]} imgSrc={project_img[i]} youtube={text[count][4]}></LeftSide>}
        </div>);
        count++;
    }
